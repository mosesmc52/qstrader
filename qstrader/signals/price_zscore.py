...
class PriceZScoreSignal(Signal):
    ...
    def _z_score(self, assets, lookback):
        ...
        X_prices = pd.Series(self.buffers.prices[self._asset_lookback_key(assets[0])])

        Y_prices = pd.Series(self.buffers.prices[self._asset_lookback_key(assets[1])])
