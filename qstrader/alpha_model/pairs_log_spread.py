...
class PairsLogSpreadAlphaModel(AlphaModel):
    ...
    def _generate_signals(self, assets, weights, dt):
        ...
        latest_price_0 = np.log(
            self.data_handler.get_asset_latest_mid_price(dt, assets[0])
        )
        latest_price_1 = np.log(
            self.data_handler.get_asset_latest_mid_price(dt, assets[1])
        )
